...
bool gui_component::load_parser_unsafe(const map<string, string>& kv_store) {
	...
	if (iterator != kv_store.end()) {
		...
		// special handling for setting dimensions to screen size
		if (iterator->second == "screen_size") {
			shared_ptr<gui_controller> controller = output_controller.lock();
			if (controller != nullptr) {
				set_dimensions_unsafe(controller->get_screen_resolution());
			} else {
				set_dimensions_unsafe(vec2d(0,0));
			}

		// setup dimensions by parsing the string
		} else {
			vec2d dimensions;
